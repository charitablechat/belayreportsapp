...
- `index.html` has an early guarded `/sw.js` registration for production, non-iframe pages. This duplicates `src/main.tsx` registration intentionally so a production page can install/update the SW even if the React bundle fails or loads late.
