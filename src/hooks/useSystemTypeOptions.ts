...
const CATEGORY = "operating_systems";
...
export function useSystemTypeOptions(existingValues: string[] = []) {
    ...
    queryFn: async () => {
      ...
      if (isOnline) {
        try {
          ...
          if (!error && data && data.length === 0) {
            await seedDefaults();
            return mergeExisting([...DEFAULT_SYSTEM_TYPES], existingValues);
          }
        } catch (e) {
          console.warn("[useSystemTypeOptions] Fetch failed, using cache:", e);
        }
      }
  ...
  const seedDefaults = async () => {
    try {
      ...
      const inserts = DEFAULT_SYSTEM_TYPES.map((label, i) => ({
      ...
      const cacheEntries = DEFAULT_SYSTEM_TYPES.map((label, i) => ({
        id: `${CATEGORY}::${label}`,
        equipment_category: CATEGORY,
        label,
        display_order: i + 1,
      ...
      console.warn("[useSystemTypeOptions] Seed failed:", e);
    }
  };
      ...
      if (isOnline) {
        try {
          ...
          if (error && !error.message?.includes("duplicate") && !error.code?.includes("23505")) {
            console.error("[useSystemTypeOptions] Insert error:", error);
          } else if (!error) {
            cacheEntry.synced = true;
            await putEquipmentTypeOption(cacheEntry);
          }
        } catch (e) {
          console.warn("[useSystemTypeOptions] Insert failed, saved offline:", e);
        }
      }
    },
    onSuccess: () => {
