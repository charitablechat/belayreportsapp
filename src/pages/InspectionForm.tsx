    ...
    if (autoGenerated.criticalActions || autoGenerated.repairsPerformed) {
      setSummary(prev => {
        ...
        if (autoGenerated.criticalActions) {
          const existingManual = prev.critical_actions?.replace(/^===.*?===\n\n/s, '').trim() || '';
          const manualNotesSection = existingManual.startsWith('=== MANUAL NOTES ===') 
            ? existingManual 
            : (existingManual ? '\n\n=== MANUAL NOTES ===\n\n' + existingManual : '');
          newSummary.critical_actions = autoGenerated.criticalActions + manualNotesSection;
        }
        
        // For repairs performed - merge with existing manual notes
        if (autoGenerated.repairsPerformed) {
          const existingManual = prev.repairs_performed?.replace(/^===.*?===\n\n/s, '').trim() || '';
          const manualNotesSection = existingManual.startsWith('=== MANUAL NOTES ===')
            ? existingManual
            : (existingManual ? '\n\n=== MANUAL NOTES ===\n\n' + existingManual : '');
          newSummary.repairs_performed = autoGenerated.repairsPerformed + manualNotesSection;
        }
