...
export async function cleanupStaleCachedPhotos(): Promise<number> {
  ...
  // Use `any` for the tx variable: `idb`'s typed `transaction('photos', ...)`
  // returns a tuple-store tx whose `.store` is typed as `undefined` against
  // the multi-store schema we feed it. We only need cursor + done here.
