...
describe('applyIncomingSummary — live-state guard', () => {
  ...
  it('accepts incoming wholesale when prev is empty placeholder', () => {
    const prev = { id: 's', training_id: 'tr' };
    const incoming = { id: 's', training_id: 'tr', observations: 'server text', updated_at: t1 };
  ...
  it('preserves populated local field over blank incoming without explicit field clear', () => {
    ...
    const r = applyIncomingSummary(prev as any, incoming as any, { source: 'server-refetch' });
    expect((r.next as any).observations).toBe('recent typed text');
  ...
  it('honours an explicit cross-device clear (incoming explicit ts > local explicit ts, value blank)', () => {
    ...
    const r = applyIncomingSummary(prev as any, incoming as any, { source: 'server-refetch' });
    expect((r.next as any).observations).toBe('');
  ...
  it('keeps local populated when both populated and incoming lacks explicit per-field stamp', () => {
    ...
    const r = applyIncomingSummary(prev as any, incoming as any, { source: 'server-refetch' });
    expect((r.next as any).recommendations).toBe('local new');
  ...
  it('accepts genuinely newer populated incoming when explicit per-field stamp is newer', () => {
    ...
    const r = applyIncomingSummary(prev as any, incoming as any, { source: 'idb-load' });
    expect((r.next as any).recommendations).toBe('cross-device newer');
  });
  ...
  it('backup-restore with blank backup cannot wipe populated local summary', () => {
    ...
    const backup = { id: 's', training_id: 'tr', observations: '', recommendations: null, updated_at: t2 };
    const r = applyIncomingSummary(prev as any, backup as any, { source: 'backup-restore' });
    expect((r.next as any).observations).toBe('live editor text');
    expect((r.next as any).recommendations).toBe('live recs');
  });
  ...
  it('TipTap empty shell <p></p> is treated as missing (not an explicit clear)', () => {
    ...
    const r = applyIncomingSummary(prev as any, incoming as any, { source: 'idb-load' });
    expect((r.next as any).observations).toBe('real text');
  });
  ...
  it('records trace entries with metadata only (no field values)', () => {
    ...
    const incoming = { id: 's', training_id: 'tr', observations: null, updated_at: t2 };
    applyIncomingSummary(prev as any, incoming as any, {
